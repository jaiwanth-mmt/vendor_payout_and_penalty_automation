...
async def process_category_batch_async(
    ...
    *,
    ...
    enriched_df = enrich_message_column(enriched_df)
...
def enrich_message_column(df: pd.DataFrame) -> pd.DataFrame:
    """Fill ``message`` deterministically from Remarks → Sub Category (no LLM, no comments)."""
    ...
    for index in output.index.tolist():
        ...
        if not (remarks or sub_category):
            output.at[index, MESSAGE_COLUMN] = ""
            continue
        output.at[index, MESSAGE_COLUMN] = build_message_from_row(
            remarks=remarks,
...
async def enrich_message_column_async(df: pd.DataFrame) -> pd.DataFrame:
    """Async-compatible wrapper; message enrichment is synchronous and deterministic."""
    return enrich_message_column(df)
