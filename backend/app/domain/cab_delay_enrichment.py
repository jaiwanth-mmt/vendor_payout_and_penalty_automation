...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Initial Azure OpenAI completion token budget. GPT-5 may use part of this for reasoning, "
            "and length retries may use a larger budget."
        ),
    )
    parser.add_argument(
        "--reasoning-effort",
...
class AzureOpenAIEmptyMessageError(RuntimeError):
    def __init__(self, *, finish_reason: object, attempts: int) -> None:
        reason = raw_tracking_value(finish_reason) or "unknown"
