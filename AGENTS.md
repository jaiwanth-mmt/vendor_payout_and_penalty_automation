...
5. Registry-dispatched category enrichers + deterministic `message` column (Remarks → Sub Category allow-list format; call comments kept for display/tools only). Common tracking always includes `amount` + `ttrip_type`. **Fulfillment Not Done / Vendor No Show** computes SOP fine: airport/local → `min(0.5 * amount, 500)`; other non-empty trip types → `min(0.25 * amount, 2000)`; writes `fine_before_sop` / `fine_after_sop` and overwrites `Recoverable` when computable. Missing `amount`/`ttrip_type` keeps workbook Recoverable and forces Needs check.
